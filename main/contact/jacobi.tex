...
The following construction is analogous to the one for contact Hamiltonian systems, with $\omega$ substituted for $\dd{\alpha}$.
...
This vector field is unique, because the kernel of $\omega$ is always one-dimensional, and $\alpha$ has at least a component in the zero direction of $\omega$. If this were not the case, the condition given above would not be satisfied. In the given coordinates, the Reeb vector field is equal to
$$ R = \pdv{}{U}. $$
...
An analogous decomposition can be made in the cotangent bundle. A \emph{semi-basic form} (the only component we are interested in) is a 1-form $\zeta$ that satisfies the condition
$$ \intpr{R}{\zeta} = 0. $$
Likewise, any form can be decomposed as follows \cite{Libermann1987}
\begin{equation}
    \zeta = \qty(\intpr{R}{\zeta})\alpha + \underbrace{\zeta - (\intpr{R}{\zeta})\alpha}_\text{semi-basic}, 
    \label{eq:form_decomposition}
\end{equation}
where the right term is semi-basic.

Because the kernel of $\omega$ and $\alpha$ do not coincide, $\omega$ defines an isomorphism between the horizontal vector fields and the semi-basic forms.Let
$$ \toDual{\omega}: \tbundle{M}\vert_{\ker \alpha} \to \ctbundle{M}\vert_\text{sb}: \quad X \mapsto \intpr{X}{\omega},$$
where `sb' refers to the cotangent bundle restricted to the semi-basic forms. This mapping is one-to-one; the inverse mapping $\fromDual{\omega}$ relates any semi-basic form to a horizontal vector field.

The bivector $\Lambda$ is then defined as
$$ \omega\qty(\fromDual{\omega}(\eta), \fromDual{\omega}(\xi)), $$
where $\eta$ and $\xi$ are semi-basic. However, we wish to apply the associated mapping $\fromDual{\omega}$ to general 1-forms ($\dd{H}$ is generally not semi-basic). Therefore, we first canonically project it to the semi-basic forms. 
