...
where $C$ is an integration constant which we can choose to be 0, so $\rho_0(t) = \ec^{\gamma t}$. a
...
\paragraph{Liouville submanifolds} In \cref{ssec:contact_dissipation} we devoted considerable attention to the fact that for the equations of motion to represent a physical trajectory, the contact Hamiltonian should numerically be equal to zero. This is equivalent to stating that the trajectories lie in Legendre submanifolds.
