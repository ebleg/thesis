...
The coordinate $\lambda$, and therefore $\rho_0$ and $\rho$, are not canonical coordinates, for they depend on the particular choice of the contact form to begin with. Indeed, in this particular choice of $\alpha_e$ one point was left out ($p = \infty$), which effectively rules all the points for which $\rho_0 = 0$. However, only the case for which both $\rho$ and $\rho_0$ vanish should be taken out of the manifold, because the other cases can be covered by picking a different coordinate chart. This space is the cotangent bundle of $Q_e$ without its zero section, denoted by $ \ctzbundle{Q_e} $.

The former discussion relies heavily on the choice of coordinate chart. Therefore, we wish to make it more mathematically precise using the language of principal bundles.
