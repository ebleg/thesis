...
The contact structure is be derived based on thermodynamic reasoning. Therefore, the next section first discusses the traditional role of contact geometry in thermodynamics, after it will be applied to dissipative mechanics.
...
\end{equation}
If this vector field is be projected to \(\pctbundle{Q_e}\)  we obtain \cref{eq:ham_vf_e_liouville}%\footnote{This is the `differential geometric' way of deriving the equation for \(\dot{p}\) as we did above.} 
 using the pushforward of the projection map \(\sigma\).
