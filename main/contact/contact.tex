...
\begin{equation}
    \Psi^{-1}_{\alpha_e}(K) = \Psi^{-1}_{\alpha}\qty(\frac{1}{f}\,K).
...
\paragraph{Damped harmonic oscillator} The contact Hamiltonian system for the damped harmonic oscillator can now be lifted to a homogeneous Hamiltonian system on the symplectified space. This lift is defined as \cite{VanderSchaft2021a,Libermann1987}
 $$ K_e(q_0, q, p) = \mathscr{H}(q_0, q, -1, \rho), $$
 or in other words
 $$ K_e(q_0, q, -\frac{\rho}{\rho_0}) = \mathscr{H}(q_0, q, \rho_0, \rho). $$


%Define the \emph{Liouville vector field} as the infinitesimal action of the group action:
%$$ Z \coloneq \dv{}{\lambda} \left . (x \raction{\lambda})\right \vert_{\lambda = 1}, \quad x \in \ctzbundle{Q_e},$$
%or in coordinates
%\begin{equation}
%    Z = \rho_0\pdv{}{q_0} + \rho\pdv{}{q}.
%\end{equation}
%
%A function $\mathscr{H} \in \functions{\ctzbundle{Q_e}}$ is called homogeneous of degree 1 if
%$$ \lied{Z}{\mathscr{H}} = \mathscr{H}. $$
%Again, in coordinates, this means that we have
%$$ \mathscr{H}(q_0, q, \lambda \rho_0, \lambda \rho) = \lambda\mathscr{H}(q_0, q, \rho_0, \rho) \qquad \lambda \in \mgroup,$$
%which is to say that it commutes with the group action $\raction{\mgroup}$.
%
%Hence
