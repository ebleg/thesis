...
It is worth pointing out that the correspondence between a hyperplane and the kernel of a 1-form is not one-to-one. Indeed, multiplying $\alpha$ by any nonzero function yields a different 1-form with the same kernel. This is to say that they are different contact forms, but they give rise to the same contact structure. This ambiguity is very important, and will play a vital role in the process of symplectification discussed in \cref{ssec:symplectification}
...
$$ \Psi_\alpha: \vfields{X}{c}{M} \to \functions{M}: \quad X_H \mapsto H = \intpr{X_H}{\coloneq} , $$
where $\vfields{X}{c}{M}$ is the collection of infinitesimal strict contactomorphisms. These are vector fields that preserve the strictly contact structure given by $\alpha$, and are subject to the following condition:
$$ \lied{X_H}{\alpha} = s \alpha, $$
where $s$ is an arbitrary smooth function on $M$. This condition is based on the fact that the same contact structure is specified by any nonzero multiple of a given contact form.
...
The form $\alpha_\text{\tiny G}$ given by Gibbs' relation is a contact form on the thermodynamic phase space, since
