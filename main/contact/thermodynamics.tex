...
\subsection{Contact geometry in classical mechanics}
...
\end{equation}
The precise nature of $\Phi$ is discussed in detail in \cref{sec:contact_ham_systems}. The `forward mapping' is defined as
$$ \Psi_\alpha(X_H) \coloneq \intpr{X_H}{\alpha}. $$
To go the other way is not so simple, since $ \Phi_\alpha^{-1}$ must map the general class of smooth functions to a special subclass of inifinitesimal exact contactomorphims $\vsfields{c}{M}$. For $X_H$ to be an infinitesimal exact contactomorphism means that it must satisfy the following condition:
$$ \lied{X_H}{\alpha} = s \alpha, $$
with $s$ some function (zeros allowed) on $M$.

The trick to find $\Phi_\alpha^{-1}(X_H)$ is to separate the Hamiltonian vector field into a \emph{horizontal} vector field $X^\text{hor}_H$  and a vertical vector field $X^\text{ver}_H$ according to the following splitting of the tangent bundle to $M$ \cite{Libermann1987}
...
The vertical part of the vector field is easy to find based on the definition of $\Phi$, and is equal to
\begin{equation}
