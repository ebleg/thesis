...
\subsection{Arguments from thermodynamics}

\textbf{First law of thermodynamics}
$$ \dd{U} = \eta - \beta $$
where $U$ is the internal energy of the system, $\eta$ the heat form and $\beta$ the work form. \cite{Frankel2012,Arnold1989b,Bamberg1988}
We shall \emph{not} denote these forms by $\eta \neq \text{\dj}Q$, because the work and heat forms are definitely not closed (and therefore not exact!). The real `power' of the first law resides in the fact that $\dd{U}$ \emph{exists} --- an arbitrary form, say $\theta$, would of course yield a meaningless statement. Indeed, the first law states that the difference between the energy and work forms is exact. Furthermore, we have that the form given by 
$$ \theta = \dd{U} - \eta + \beta $$
pulls back to zero.

The system boundary contains both the mass, the spring and the fluid contained in the damper. One may abstract this damper by considering it to be a thermal heat bath connected to the system, where the damper system merely acts as a connecting interface between the two. The internal energy of the system is:
$$ U = \text{mechanical energy} + \text{internal energy of the damper}  $$
Naturally, the mechanical energy of the system is equal to the sum of the kinetic energy stored in the mass, and the potential energy stored in the spring. The internal energy of the damper fluid is a function of the entropy of the fluid, or, as we will see later, the temperature of the fluid.

Gibbs 1-form
$$ \theta = \dd{U} - T\dd{S} - P\dd{V} $$

Equilibrium question: some (Hermann) define $\varphi^*(\theta)$ as a condition for equilibrium conditions, which is not necessarily the case (right).
