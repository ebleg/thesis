...
The great advantage of using the purely symplectic formalism on the lifted space is the fact that the Hamiltonian is invariant under the flow it generates, since the explicit time-dependence has been removed:
...
$$ \mathscr{H} = -\rho_0\,\underbrace{\qty(\mathscr{H}_0\,\rho_0^{-1})}_{H} \qquad \mathscr{H}_0 \in \real.$$
Recall that we made the assumption earlier that $\rho_0$ is a function \emph{without zeros}. The constant $\mathscr{H}_0$ is a degree of freedom in the system that we are free to choose, for the equations of motion will be consistent with any chosen value. This is 
