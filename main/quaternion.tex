...
\begin{equation}
    ...
    \label{eq:quat_norm}
\end{equation}
As mentioned, this norm is not positive definite, in stark contrast to quaternions or complex numbers. Split-quaternions can be categorized into three regimes based on their norm being negative, zero or positive. In the tradition of special relativity, these regimes are named \emph{spacelike}, \emph{lightlike} and \emph{timelike} respectively: \cite{Misner1970,Landau1971}
\begin{itemize}
    \item \textbf{Timelike}: $ \mathscr{N}(a) > 0 $, with length $ \norm{a} = \sqrt{a \conj{a}}$. 
    \item \textbf{Lightlike}: $ \mathscr{N}(a) = 0 $, with length $\norm{a} = 0$. 
    \item \textbf{Spacelike}: $ \mathscr{N}(a) < 0 $, with length $\norm{a} = \sqrt{\abs{a\conj{a}}}$.
\end{itemize}
...
The above expression is equivalent the Lorentz norm applied to a vector in $\real^3$; we will denote $\real^3$ equipped with the Lorentz norm by $\real^{2, 1}$. \cite{Jafari2014} Observe that this quadratic form is not positive-definite either; in the the same veign as before, we can therefore classify quaternions by the `sign' of their vector part again. We refer to these regimes as \emph{timelike (vectors)}, \emph{spacelike (vectors)} and \emph{lightlike (vectors)} in the same way. 
...
\begin{table}[ht]
    ...
    \begin{tabular}{c|cccc}
        \toprule
        &  & \multicolumn{3}{c}{$ \mathscr{N}_v(\vec{a}) $} \\
        \hline
        & & & & \\[-1ex]
...
\begin{itemize}
    ...
    \item The eigenvalues of a $2\times2$-matrix can be expressed in terms of its trace and its determinant:
        ...
        \begin{equation}
            ...
            \begin{cases}
                a_0 \pm \ii\norm{\vec{a}} & \vec{a} \text{ timelike}\\
                a_0  & \vec{a} \text{ lightlike}\\
...
\subsubsection*{Timelike split-quaternion norm}
    \begin{itemize}
        ...
        We can also relate the vector norm to the Jordan form of the associated matrix. Recall that the Jordan form is `special' if a matrix is not completely diagonalizable: in the case of two identical eigenvalues, their geometric multiplicity is equal to one instead of two. In terms of the corresponding matrix, the vector norm is equal to: 
            $$ \mathscr{N}_v(\vec{a}) = \det(A - \frac{\trace(A)}{2}) $$
...
$$ \exp(a) = \exp(a_0)\,\exp(\quatvec{a_1}{a_2}{a_3}). $$
...
$$ \mathscr{N}_v(\vec{a}) = \vec{a} \conj{\vec{a}} = -\vec{a}^2. $$
Furthermore, consider the \emph{unit} split-quaternion
$$ \hat{\vec{a}} = \frac{\vec{a}}{\sqrt{\abs{\mathscr{N}_v(\vec{a})}}} $$
which squares to
$$ \hat{\vec{a}}^2 = - \mathscr{N}_v(\hat{\vec{a}}) 
                   = - \frac{\mathscr{N}_v(\vec{a})}{\abs{\mathscr{N}_v(\vec{a})}} 
                   = - \sgn(\mathscr{N}_v(\vec{a}))
$$
...
\begin{equation}
    \begin{split}
        \exp(a) &= \exp(a_0)\qty(\sum_{k=0}^n \frac{1}{k!} \vec{a}^k) \\
                &= \exp(a_0)\qty[\sum_{k=0}^n \frac{\qty(\vec{a}^2)^k}{(2k)!}  + \sum_{k=0}^n \frac{\vec{a}\qty(\vec{a}^2)^k}{(2k+1)!}] \\
                &= \exp(a_0)\qty[\sum_{k=0}^n \frac{\norm{\vec{a}}^{2k}\, \qty(\uvec{a}^2)^k}{(2k)!}  + \uvec{a}\sum_{k=0}^n \frac{\norm{\vec{a}}^{2k + 1} \, \qty(\uvec{a}^2)^k}{(2k+1)!}] \\
    \end{split}
\end{equation}
We must know acknowledge the three possiblities that were identified before
\begin{itemize}
    \item If $\vec{a}$ is \emph{timelike}, then the above expression reverts to
        \begin{equation}
            \begin{split}
                \exp(a) &= \exp(a_0)\qty[\sum_{k=0}^n \frac{\norm{\vec{a}}^{2k}\, \qty(-1)^k}{(2k)!}  + \uvec{a}\sum_{k=0}^n \frac{\norm{\vec{a}}^{2k + 1} \, \qty(-1)^k}{(2k+1)!}] \\
                        &= \exp(a_0)\qty[\cos(\norm{a})  + \uvec{a}\sin(\norm{a})] \\
    ...
    \item lightlike $0^0$??
        \begin{equation}
            \begin{split}
                \exp(a) &= \exp(a_0)\qty[\sum_{k=0}^n \frac{\norm{\vec{a}}^{2k}\, \qty(0)^k}{(2k)!}  + \uvec{a}\sum_{k=0}^n \frac{\norm{\vec{a}}^{2k + 1} \, \qty(0)^k}{(2k+1)!}] \\
                        &= \exp(a_0)\qty[\cos(\norm{a})  + \uvec{a}\sin(\norm{a})] \\
            \end{split}
        \end{equation}
\end{itemize}
...
\begin{table}[ht!]
    ...
    \begin{tabular}{llll}
        ...
        \midrule
            ...
            Natural frequency & $\Omega$ & $\sqrt{k/m}$ & \si{\per \second} \\
        \bottomrule
    \end{tabular}
\end{table}
