...
The distinctive feature that sets split-quaternions apart from conventional quaternions resides in the diagonal elements of \cref{tab:quat_table}. For quaternions, all basis elements but 1 square to \(-1\), which is not the case for the split-quaternions (only \quati does). This is also the reason why split-quaternions are `split', for this difference in sign makes their norm (to be defined later) into an indefinite quadratic form. That is to say, whereas quaternions have a `metric signature' (in a very imprecise sense of the word metric) of \((+, +, +, +)\), the split-quaternions have \((+, +, -, -)\). The difference `metric' signature makes the algebra of split-quaternions different from its conventional quaternion counterpart.
...
\end{equation}
For convenience, the vector part will be referred to as the symbol of the split-quaternion in boldface. Hence, we have $\vec{a} = \vecpart{a}$, and
\begin{equation}
     a = a_0 + \vec{a}.
\end{equation}

In addition, every split-quaternion has a unique \emph{conjugate} split-quaternion defined as
\begin{equation}
    \begin{split}
...
The split-quaternion norm is (in stark contrast to the norm defined for quaternions) \emph{indefinite}: it can be positive, negative or zero. Split-quaternions can therefore be categorized into three regimes based on the sign of their squared norm. In the tradition of special relativity, these regimes are named\footnote
{Like the space of split-quaternions, spacetime is also four-dimensional. However, the signature of the Minkowski metric is different from the split-quaternion signature: it is either \((-, +, +, +)\) or equivalently \( (+, -, -, -)\) depending on the sign convention one chooses to observe. The terminology (i.e. spacelike, timelike, lightlike) applies nonetheless.} \cite{Misner1970,Landau1971}
\begin{itemize}
...
\begin{equation}
     \norm{a} \coloneq \sqrt{\mathscr{N}(a)}.
\end{equation}
Because of the indefiniteness of $\mathscr{N}$, $\norm{a}$ is imaginary if $a$ is spacelike.
...
\end{equation}
This is not an abuse of notation: \(\vec{a}\) simply refers to the split-quaternion with the same vector part as \(a\) but with a zero scalar part. We can therefore use the same function with no ambiguity. The \emph{vector norm} is then 
\begin{equation}
    \norm{\vec{a}} \coloneq \sqrt{\mathscr{N}(\vec{a})}. 
\end{equation}

In the remainder of this chapter, the distinction between the squared norm of the entire split-quaternion and its squared vector norm plays a crucial role. Therefore, we will refer to \(\mathscr{N}(a)\) as the \emph{squared total norm}, and to \(\mathscr{N}(\vec{a})\) as the squared vector norm. Likewise, we refer to $\norm{a}$ as the \emph{total norm}, and to $\norm{\vec{a}}$ as the vector norm.

The quadratic form given in \cref{eq:vector_norm} is not positive-definite either. As a result, we can also establish a classification based on the vector norm instead of the total norm:
\begin{itemize}
...
The squared vector norm is not completely independent from the squared total norm: observe that 
\begin{equation}
    \mathscr{N}(a) < 0\quad \Rightarrow \quad \mathscr{N}(\vec{a}) < 0.
\end{equation} 
As such, \emph{a spacelike split-quaternion always has a spacelike vector part}. The converse is not necessarily true. Likewise, a lightlike split-quaternion can only have a lightlike or spacelike vector part, i.e.
\begin{equation}
    \mathscr{N}(a) = 0 \quad \Rightarrow \quad \mathscr{N}(\vec{a}) \leq 0
\end{equation}
These two statements rule out three hypothetical combinations for the total norm and vector norm (spacelike - timelike, spacelike - lightlike and lightlike - timelike). Hence, six possible combinations remain; they are listed in \cref{tab:class_combinations}. 
...
The classification given by \cref{tab:class_combinations} is very important because it corresponds directly to the qualitative classification of dynamical systems. This is the subject of \cref{ssec:system_classification}.

In the remainder of this chapter, the split-quaternion properties that are defined above are used profusely. Therefore, \cref{tab:spquat_properties} provides an overview of these properties and their definitions. 
...
\begin{table}[ht!]
    ...
    \begin{tabular}{lll}
        ...
        \midrule
        Squared (total) norm & \(\mathscr{N}(a)\) & \(a\conj{a} = a_0^2 + a_1^2 - a_2^2 - a_3^2\) \\
        (Total) norm & \(\norm{a}\) & \(\sqrt{\mathscr{N}(a)}\) \\
        \midrule
        ...
        Vector norm & \(\norm{\vec{a}}\) & \(\sqrt{\mathscr{N}(\vec{a})}\) \\
        \bottomrule
    \end{tabular}
\end{table}
\renewcommand{\arraystretch}{1}
 
\subsubsection{Lorentzian three-space}
In contrast to the split-quaternion itself, the vector part of the split-quaternion does live in a space with a Lorentz structure. The \emph{Lorentzian $n$-space} is the real vector $\real^n$ space equipped with the \emph{Lorentzian inner product}. 
\begin{equation}
     \lorinner{\vec{a}}{\vec{b}} \coloneq a_1b_1 \, - \, a_2b_2 \, - \, \ldots \, - \, a_nb_n \qquad \vec{a}, \vec{b} \in \real^n.
\end{equation}
Since the Lorentzian inner product has signature $(+, -, \ldots, -)$, the Lorentzian $n$-space is denoted by $\real^{1, n-1}$. For $n = 4$, the Lorentzian space is also called the \emph{Minkowski space}, which forms the setting of special relativity. 

Based on the Lorentzian inner product, the \emph{Lorentz norm} can be defined as follows:
\begin{equation}
    \norm{\vec{a}} = \sqrt{\lorinner{\vec{a}}{\vec{a}}},
\end{equation}
which coincides with the vector norm for split-quaternions defined above on the Lorentzian 3-space $\real^{1,2}$.
...
The split-quaternion product can be conveniently expressed in terms of the Lorentzian inner product and cross product. For two split-quaternions $a$ and $b$ with their respective vector parts $\vec{a}$ and $\vec{b}$, we have
\begin{equation}
...
\begin{itemize}
    ...
    \item The eigenvalues of a \(2\times2\)-matrix can be expressed in terms of its trace and its determinant:
        ...
        Hence, the real part and the squared norm of the vector parts of the split-  quaternion coincide with the real and imaginary parts of the eigenvalues of the matrix. When the vector either is spacelike or timelike, the $\ii$ cancels, and the overall eigenvalue is real.
\end{itemize}
