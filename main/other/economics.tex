...
The pairing of flows and amounts of goods is specified by the \emph{vertical isomorphism} $S$, which is a tensor of valence \((1,1)\) on \(\tbundle{Q}\): \cite{Carinena1990}
\begin{equation}
    S = \pdv{}{\dot{q}^i}\otimes \dd{q^i},
\end{equation}
where $\otimes$ is the tensor product. Using the vertical isomorphism, the \emph{Lagrange 1-form} $\theta_L$ is defined as
\begin{equation}
