...
\begin{equation}
    \alpha_L \equiv \dd{L} \circ S = \sum_{j=1}^n \pdv{L}{v^j}\dd{q^j}.
\end{equation}
Observe that the Lagrange 1-form is also equal to the pullback of the Liouville form under the Legendre transformation: $\alpha_L = (\fiberder{L})^* \alpha$. \cite{Abraham1978}
Secondly, we define the \emph{Lagrange 2-form} as: \cite{Abraham1978,Carinena1990}
\begin{equation}
    \omega_L \equiv - \dd{\alpha}_L = \pdv[2]{L}{v^i}{v^j}\wedgep{\dd{q^j}}{\dd{v^i}} + \pdv[2]{L}{q^i}{v^j}\wedgep{\dd{q^j}}{\dd{q^i}}.
\end{equation}
...
\begin{equation*}
    \begin{split}
        \lied{X_L}{\alpha_L} &= \dd{L} \\
        \dd{\qty(\intpr{X_L}{\alpha_L})} + \intpr{X_L}{\dd{\alpha_L}} &= \dd{L} \\
        \dd{\qty(\intpr{X_L}{\alpha_L})} - \intpr{X_L}{\omega_L} &= \dd{L}
    \end{split}
\end{equation*}
The fact that $X_L$ is second-order implies that $\intpr{X_L}{\alpha_L} = Z(L)$. Therefore 
\begin{equation*}
    \begin{split}
